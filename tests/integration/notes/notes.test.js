...
describe("POST /api/v1/:projectId/ — Create Note", () => {
  ...
  test.todo(
    "should default note status to PERSONAL when status is not provided",
  );
...
describe("GET /api/v1/:projectId/ — Get Notes", () => {
  test.todo(
    "should return 200 and include SHARED notes for all project members",
  );

  test.todo(
    "should return PERSONAL notes only to the note owner, not other members",
  );
...
describe("GET /api/v1/:projectId/note/:noteId — Get Note By ID", () => {
  ...
  test.todo(
    "should return 404 when a non-owner tries to fetch someone else's PERSONAL note",
  );
...
describe("PUT /api/v1/:projectId/note/:noteId — Update Note", () => {
  ...
  test.todo(
    "should return 403 when a different project member tries to update someone else's note",
  );
...
describe("DELETE /api/v1/:projectId/note/:noteId — Delete Note", () => {
  ...
  test.todo(
    "should return 403 when a different project member tries to delete someone else's note",
  );
...
describe("PATCH /api/v1/:projectId/note/:noteId/status — Change Note Status", () => {
  test.todo(
    "should return 200 when owner changes status from PERSONAL to SHARED",
  );

  test.todo(
    "should return 200 when owner changes status from SHARED to PERSONAL",
  );

  test.todo(
    "should return 400 when owner tries to set the same status that is already set",
  );

  test.todo(
    "should return 403 when a non-owner tries to change the note status",
  );

  test.todo(
    "should return 400 when status value is not one of: personal, shared",
  );
});
