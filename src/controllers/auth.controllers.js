...
const getAllUsers = asyncHandler(async (req, res) => {
  const users = await User.find().select(
    "-password -refreshToken -__v -_id -emailVerificationToken -emailVerificationTokenExpiry -createdAt -updatedAt -refreshTokenExpiry -forgotPasswordToken -forgotPasswordTokenExpiry -isEmailVerified -avatar",
  );
  res
    .status(200)
...
const getCurrentUser = asyncHandler(async (req, res) => {
  ...
  const user = await User.findById(userId).select(
    "-password -refreshToken -__v -emailVerificationToken -emailVerificationTokenExpiry -refreshTokenExpiry -forgotPasswordToken -forgotPasswordTokenExpiry",
  );
  if (!user) {
    throw new ApiError(404, "User not found");
  }
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  if (!incomingRefreshToken) {
    throw new ApiError(
      401,
      "Refresh token is missing in refresh token cookie or Authorization header",
    );
  }
